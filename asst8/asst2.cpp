...
static vector<RigTForm> prevFrame, curFrame, nextFrame, nnextFrame;
static float g_deformSpeed = 1.0;
...
static shared_ptr<Mesh> g_mesh, g_originMesh;
static shared_ptr<SimpleGeometryPN> g_PN;
...
static void printCvec3(Cvec3 v) {
	cout << "printCvec3: [" << v[0] << ", " << v[1] << ", " << v[2] << "]" << endl;
}

static void updateMesh() {
	for (int i = 0; i < g_mesh->getNumVertices(); i++) {
		Mesh::Vertex v = g_mesh->getVertex(i);
		v.setNormal(Cvec3(0, 0, 0));
	}
	for (int i = 0; i < g_mesh->getNumVertices(); i++) {
		Mesh::Vertex v = g_mesh->getVertex(i);
		Mesh::VertexIterator viter = v.getIterator();
		Mesh::VertexIterator viter0 = viter;
		Cvec3 accumVec = viter.getFace().getNormal();
		/*cout << "vertex normals" << endl;
		printCvec3(viter.getFace().getNormal());*/
		++viter;
		while (viter != viter0) {
			accumVec += viter.getFace().getNormal();
			//printCvec3(viter.getFace().getNormal());
			++viter;
		}
		v.setNormal(normalize(accumVec));
	}
	...
	for (int i = 0; i < g_mesh->getNumFaces(); i++) {
		...
		VertexPN vpn0, vpn1, vpn2, vpn3;
		if (g_shading == 0) {
			vpn0 = VertexPN(v0.getPosition(), v0.getNormal());
			vpn1 = VertexPN(v1.getPosition(), v1.getNormal());
			vpn2 = VertexPN(v2.getPosition(), v2.getNormal());
			vpn3 = VertexPN(v3.getPosition(), v3.getNormal());
		}
		else {
			vpn0 = VertexPN(v0.getPosition(), f.getNormal());
			vpn1 = VertexPN(v1.getPosition(), f.getNormal());
			vpn2 = VertexPN(v2.getPosition(), f.getNormal());
			vpn3 = VertexPN(v3.getPosition(), f.getNormal());
		}
		/*cout << "Face [" << i << "] vertices" << endl;
		printCvec3(v0.getPosition());
		printCvec3(v1.getPosition());
		printCvec3(v2.getPosition());
		printCvec3(v3.getPosition());*/
		vtx.push_back(vpn0);
		vtx.push_back(vpn1);
		vtx.push_back(vpn2);
		vtx.push_back(vpn0);
	...
	g_PN->upload(&vtx[0], vtx.size());
}

static void subdivision(shared_ptr<Mesh> h, int cnt) {
	while (cnt != 0) {
		for (int i = 0; i < h->getNumFaces(); i++) {
			Mesh::Face f = h->getFace(i);
			Cvec3 pos = Cvec3();
			for (int j = 0; j < f.getNumVertices(); j++) {
				pos += f.getVertex(j).getPosition();
			}
			h->setNewFaceVertex(f, pos / f.getNumVertices());
		}
		for (int i = 0; i < h->getNumEdges(); i++) {
			Mesh::Edge e = h->getEdge(i);
			Cvec3 pos = Cvec3();
			pos += e.getVertex(0).getPosition();
			pos += e.getVertex(1).getPosition();
			pos += h->getNewFaceVertex(e.getFace(0));
			pos += h->getNewFaceVertex(e.getFace(1));
			h->setNewEdgeVertex(e, pos / 4);
		}
		for (int i = 0; i < h->getNumVertices(); i++) {
			Mesh::Vertex v = h->getVertex(i);
			Mesh::VertexIterator viter = v.getIterator();
			Mesh::VertexIterator viter0 = viter;
			Cvec3 fpos = h->getNewFaceVertex(viter.getFace());
			Cvec3 vpos = viter.getVertex().getPosition();
			int num = 1;
			++viter;
			while (viter != viter0) {
				fpos += h->getNewFaceVertex(viter.getFace());
				vpos += viter.getVertex().getPosition();
				++num;
				++viter;
			}
			h->setNewVertexVertex(v, v.getPosition() * (num - 2) / num + (vpos + fpos) / num / num);
		}
		h->subdivide();
		--cnt;
	}
...
static void animateTimerCallbackMesh(int ms) {
	g_mesh.reset(new Mesh(*g_originMesh));
	float t = (float)ms / (float)g_msBetweenKeyFrames;
	for (int i = 0; i < g_mesh->getNumVertices(); i++) {
		Mesh::Vertex v = g_mesh->getVertex(i);
		Mesh::Vertex ov = g_originMesh->getVertex(i);
		v.setPosition(v.getPosition() * (2 + 1.75 * sin(t * (i + 1))));
	}
	subdivision(g_mesh, sublevel);
	updateMesh();
	glutTimerFunc(1000 / g_animateFramesPerSecond, animateTimerCallbackMesh, ms + 2000 / g_animateFramesPerSecond * g_deformSpeed);
	glutPostRedisplay();
}
...
static void initMesh() {
	Mesh h;
	h.load("cube.mesh");
	g_mesh.reset(new Mesh(h));
	g_originMesh.reset(new Mesh(h));
	g_PN.reset(new SimpleGeometryPN());
	updateMesh();
	animateTimerCallbackMesh(0);
}
...
static void keyboard(const unsigned char key, const int x, const int y) {
	...
	case '0':
		++sublevel;
		cout << "subdivision level: " << sublevel << endl;
		break;
	case'9':
		if (sublevel > 0) {
			--sublevel;
	...
	case '7':
		g_deformSpeed /= 2;
		cout << "deform speed: " << g_deformSpeed << endl;
		break;
	case '8':
		g_deformSpeed *= 2;
		cout << "deform speed: " << g_deformSpeed << endl;
...
static void initScene() {
	...
	g_meshNode.reset(new SgRbtNode());
	g_meshNode->addChild(shared_ptr<MyShapeNode>(new MyShapeNode(g_PN, g_specularMat)));
