...
static Matrix4 g_objectRbt2[1] = { Matrix4::makeTranslation(Cvec3(1,0,0)) }; 
...
static void motion(const int x, const int y) {
  ...
  if (g_mouseClickDown) {
      if (g_activeObj != 0 && g_viewPoint == 0) { // cube-sky frame
          if (g_activeObj == 1) {
              A = transFact(g_objectRbt[0]) * linFact(g_eyeRbt);
              g_objectRbt[0] = A * m * inv(A) * g_objectRbt[0];
          }
          else {
              A = transFact(g_objectRbt2[0]) * linFact(g_eyeRbt);
          ...
          if (g_activeObj == 1) {
              ...
              if (g_viewPoint == 1) { // manipulate 2 & view 1
                  ...
                  m = transFact(m) * inv(linFact(m));
                  g_objectRbt2[0] = g_objectRbt2[0] * m;
              }
          }
      }
