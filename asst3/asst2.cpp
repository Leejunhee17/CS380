...
static void drawStuff() {
  ...
  if (g_viewPoint == 1) {
      g_eyeRbt = g_objectRbt;
  }
  else if (g_viewPoint == 2) {
      g_eyeRbt = g_objectRbt2;
  }
  else {
      g_eyeRbt = g_skyRbt;
  }
  ...
  // draw cubes
  // ==========
  MVM = invEyeRbt * rigTFormToMatrix(g_objectRbt);
  NMVM = normalMatrix(MVM);
  sendModelViewNormalMatrix(curSS, MVM, NMVM);
  safe_glUniform3f(curSS.h_uColor, g_objectColors[0][0], g_objectColors[0][1], g_objectColors[0][2]);
  g_cube->draw(curSS);

  MVM = invEyeRbt * rigTFormToMatrix(g_objectRbt2);
  NMVM = normalMatrix(MVM);
  sendModelViewNormalMatrix(curSS, MVM, NMVM);
  ...
  bool draw = true;
  if (g_viewPoint == 0) {
      if (g_activeObj == 0 && g_worldSky == 1) {
          draw = false;
      }
  }
  else {
      if (g_activeObj == 0 || g_viewPoint == g_activeObj) {
          draw = false;
      }
  }
  if (draw) {
      glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
      g_arcballRbt = RigTForm(Cvec3(0, 0, 0), Quat(1, 0, 0, 0));
      if (g_activeObj == 1) {
          g_arcballRbt = g_objectRbt;
      }
      else if (g_activeObj == 2) {
          g_arcballRbt = g_objectRbt2;
      }
      MVM = invEyeRbt * rigTFormToMatrix(g_arcballRbt);
...
static void motion(const int x, const int y) {
  ...
  if (g_mouseLClickButton && !g_mouseRClickButton) { // left button down?
      ...
      if (g_viewPoint == 0) {
          ...
          if (g_viewPoint == g_activeObj) {
              arc = false;
          }
      }
      if (!arc){ // basic rotation when view=active in cube and sky-sky
          Quat q;
  ...
  RigTForm A;
  if (g_mouseClickDown) {
      if (g_activeObj != 0 && g_viewPoint == 0) { // cube-sky frame
          if (g_activeObj == 1) {
              A.setTranslation(g_objectRbt.getTranslation());
              A.setRotation(g_eyeRbt.getRotation());
              g_objectRbt = A * m * inv(A) * g_objectRbt;
          }
          else {
              A.setTranslation(g_objectRbt2.getTranslation());
              A.setRotation(g_eyeRbt.getRotation());
              g_objectRbt2 = A * m * inv(A) * g_objectRbt2;
          }
      }
      else if (g_activeObj == 0 && g_viewPoint == 0) { // world-sky or sky-sky
          if (g_worldSky == 1) { // sky-sky
      ...
      else if (g_activeObj != 0 && g_viewPoint != 0) { // cube-cube frame
          if (g_activeObj == 1) {
              if (g_viewPoint == 1) { // manipulate 1 & view 1
                  m.setRotation(inv(m.getRotation()));
                  g_objectRbt = g_objectRbt * m;
              }
              else { // manipulate 1 & view 2
                  A.setTranslation(g_objectRbt.getTranslation());
                  A.setRotation(g_eyeRbt.getRotation());
                  g_objectRbt = A * m * inv(A) * g_objectRbt;
              }
              ...
              if (g_viewPoint == 1) { // manipulate 2 & view 1
                  A.setTranslation(g_objectRbt2.getTranslation());
                  A.setRotation(g_eyeRbt.getRotation());
                  g_objectRbt2 = A * m * inv(A) * g_objectRbt2;
              }
              else { // manipulate 2 & view 2
                  m.setRotation(inv(m.getRotation()));
                  g_objectRbt2 = g_objectRbt2 * m;
              }
          }
      }
