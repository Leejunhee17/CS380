...
static float g_wScale = 1.0;
static float g_hScale = 1.0;
static float g_texTemp = 1.0;
...
struct ShaderState {
  ...
  GLint h_uTexUnit0, h_uTexUnit1, h_uTexUnit2;
  ...
  ShaderState(const char* vsfn, const char* fsfn) {
    ...
    h_uwScale = safe_glGetUniformLocation(h, "uwScale");
    h_uhScale = safe_glGetUniformLocation(h, "uhScale");
    h_uTime = safe_glGetUniformLocation(h, "uTime");
    h_Angle = safe_glGetUniformLocation(h, "uAngle");
...
static void display(void) {
  ...
  if (g_texTemp == 1.) {
      ...
      safe_glUniform1i(curSS.h_uTexUnit1, 2);
  }
  safe_glUniform1f(curSS.h_uVertexScale, g_objScale);

  float timeMilliseconds = glutGet(GLUT_ELAPSED_TIME);
  safe_glUniform1f(curSS.h_uwScale, g_wScale);
  safe_glUniform1f(curSS.h_uhScale, g_hScale);
  safe_glUniform1f(curSS.h_uTime, timeMilliseconds);
  safe_glUniform1f(curSS.h_Angle, g_Angle);
...
static void reshape(int w, int h) {
  g_width = w;
  g_height = h;
  g_wScale = (float)min(w, h) / w;
  g_hScale = (float)min(w, h) / h;
  glViewport(0, 0, w, h);
  glutPostRedisplay();
}
...
void keyboard(unsigned char key, int x, int y) {
  ...
  case 'h':
      cout << " ============== H E L P ==============\n\n"
          << "h\t\thelp menu\n"
          << "s\t\tsave screenshot\n"
          << "r\t\trotate 45 degree counter-clockwise\n"
          << "t\t\treset\n"
          << "drag right mouse to change square size\n"
          << "click left mouse to change texture\n";
    break;
  case 'q':
    exit(0);
  case 's':
...
static void initTextures() {
  ...
  loadTexture(*g_tex2, "thunder.ppm");
